...
st.title("📊 Dynamic Hotel Booking Chart Builder")
...
if uploaded_file:
    ...
    def get_room_type(row):
        return 'Desired' if row['reserved_room_type'] == row['assigned_room_type'] else 'Undesired'

    if all(col in df.columns for col in ['adults', 'children', 'babies']):
        df['member_type'] = df.apply(get_member_type, axis=1)

    if all(col in df.columns for col in ['reserved_room_type', 'assigned_room_type']):
        df['room_type'] = df.apply(get_room_type, axis=1)

    # Filters
    st.sidebar.header("🔍 Filter Data")
    for col in ['hotel', 'arrival_date_year', 'arrival_date_month', 'is_canceled']:
    ...
    def generate_pdf_conclusions(concs):
        ...
        pdf.cell(0, 10, "Hotel Booking Summary:", ln=True)
